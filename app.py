...
def login():
    ...
    rows = db.session.query(
        ...
        JobOffer.latitude.isnot(None),
        JobOffer.longitude.isnot(None)
    ).group_by(
        JobOffer.latitude, JobOffer.longitude, JobOffer.city
    ).all()

    public_counts = [(r.latitude, r.longitude, r.cnt, r.city) for r in rows]
...
def admin_dashboard():
    ...
    latest_users = User.query.limit(5).all()  # no order
    latest_jobs = JobOffer.query.limit(5).all()  # no order
...
def edit_job(offer_id):
    ...
    if request.method == "POST":
        ...
        off.latitude = float(request.form.get("latitude") or 0) or None
        off.longitude = float(request.form.get("longitude") or 0) or None
        db.session.commit()
...
def job_list():
    ...
    markers = [
        {"lat": r.latitude, "lng": r.longitude, "city": r.city, "count": r.cnt}
        for r in map_rows
    ]
